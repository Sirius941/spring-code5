package com.example.springcode5.config;

import java.util.List;

import com.example.springcode5.filter.TokenFilter;
import com.example.springcode5.handle.AuthenticationExceptionHandler;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(permitAllPaths)) {
            permitAllPaths.forEach(path -> {
                try {
                    //http.authorizeHttpRequests(authorizeHttpRequests->authorizeHttpRequests.requestMatchers(path).permitAll().anyRequest().authenticated());
